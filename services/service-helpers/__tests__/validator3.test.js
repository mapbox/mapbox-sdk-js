...
const { v, validate } = require('../validator3');
...
describe('v.oneOf', () => {
  ...
  describe('required', () => {
    test('success', () => {
      expect(
        t(v.shapeOf({ foo: v.required(v.oneOf('aa', 'bb')) }))({ foo: 'aa' })
      ).toEqual();
      expect(
        t(v.shapeOf({ foo: v.required(v.oneOf('aa', 'bb')) }))({ foo: 'bb' })
      ).toEqual();
    });

    test('failure', () => {
      expect(
        t(v.shapeOf({ foo: v.required(v.oneOf('aa', 'bb')) }))({})
      ).toEqual(['foo', expect.any(Function)]);
    });
  });

  describe('inside array', () => {
    var check = t(v.arrayOf(v.required(v.oneOf('aa', 'bb'))));
...
describe('v.required', () => {
  test('string', () => {
    var check = v.warn(v.required(v.string));
    expect(() => check()).toThrowError('value is required.');
  });

  test('number', () => {
    var check = v.warn(v.required(v.number));
    expect(() => check()).toThrowError('value is required.');
  });

  test('range', () => {
    var check = v.warn(v.required(v.range([0, 10])));
    expect(() => check(null)).toThrowError('value is required.');
  });

  test('boolean', () => {
    var check = v.warn(v.required(v.boolean));
    expect(() => check()).toThrowError('value is required.');
  });

  test('coordinates', () => {
    var check = v.warn(v.required(v.coordinates));
    expect(() => check()).toThrowError('value is required.');
  });

  test('date', () => {
    var check = v.warn(v.required(v.date));
    expect(() => check()).toThrowError('value is required.');
  });

  test('plainArray', () => {
    var check = v.warn(v.required(v.plainArray));
    expect(() => check()).toThrowError('value is required.');
  });

  test('plainObject', () => {
    var check = v.warn(v.required(v.plainObject));
    expect(() => check()).toThrowError('value is required.');
  });

  test('equal', () => {
    var check = v.warn(v.required(v.equal));
  ...
  test('v.oneOf', () => {
    var check = v.warn(
      v.shapeOf({
        prop: v.shapeOf({
          person: v.shapeOf({ name: v.required(v.oneOf('Jack', 'Daniels')) })
        })
      })
    );
  ...
  test('v.oneOfType', () => {
    var check = v.warn(
      v.shapeOf({
        prop: v.shapeOf({
          person: v.shapeOf({
            weight: v.required(v.oneOfType(v.string, v.range([0, 100])))
          })
        })
      })
    );
  ...
  describe('v.shapeOf', () => {
    test('simple object', () => {
      var check = v.warn(v.required(v.shapeOf({ prop: v.string })));
    ...
    test('required prop', () => {
      var check = v.warn(v.required(v.shapeOf({ prop: v.required(v.string) })));
    ...
    test('nested required prop', () => {
      var check = v.warn(
        v.shapeOf({
          prop: v.shapeOf({ person: v.shapeOf({ name: v.required(v.string) }) })
        })
      );
    ...
    test('nested object with array', () => {
      var check = v.warn(
        v.shapeOf({
          prop: v.shapeOf({
            person: v.arrayOf(
              v.shapeOf({ name: v.arrayOf(v.required(v.string)) })
            )
          })
        })
      );
    ...
    test('nested object with required oneOfType', () => {
      var check = v.warn(
        v.shapeOf({
          prop: v.shapeOf({
            person: v.shapeOf({
              name: v.required(v.oneOfType(v.arrayOf(v.string), v.plainObject))
            })
          })
        })
      );
  ...
  describe('v.arrayOf', () => {
    test('simple array', () => {
      var check = v.warn(v.required(v.arrayOf(v.string)));
    ...
    test('array with nested object', () => {
      var check = v.warn(
        v.arrayOf(v.required(v.shapeOf({ prop: v.required(v.equal('c')) })))
      );
