...
describe('appendQueryObject', () => {
  ...
  test('preserves array for setfilter', () => {
    expect(
      appendQueryObject('/foo/bar', {
        setfilter: ['==', 'name_en', 'Canada']
      })
    ).toBe(
      '/foo/bar?setfilter=%5B%22%3D%3D%22%2C%22name_en%22%2C%22Canada%22%5D'
    );
  });
  ...
  test('accepts objects', () => {
    expect(
      ...
      '/foo/bar?test=%7B%22one%22%3A%22two%22%2C%22three%22%3A%22four%22%7D'
    );
  });
