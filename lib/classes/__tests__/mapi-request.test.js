...
describe('MapiRequest#url', () => {
  ...
  test('works with an access token, appending query (with object), interpolating route params, prepending origin', () => {
    const client = createMockClient();
    const request = new MapiRequest(client, {
      path: '/foo/:a/:b/bar',
      method: 'MOCK_METHOD',
    ...
    const token = tu.mockToken();
    expect(request.url(token)).toBe(
      `mockClientOrigin/foo/yep/nope/bar?fish=tuna&myObj=%7B%22one%22%3A%22two%22%2C%22three%22%3A%22four%22%7D&access_token=${token}`
    );
  });
});
